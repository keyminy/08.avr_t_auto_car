...
#define SLA_W (0x27 << 1) //I2C LCD주소는 0x27인데 << 1로 하는 이유는 write모드를 유지하기 위함 

void I2C_LCD_Test()
{
		//I2C_write_byte(SLA_W, '9'); 
		I2C_LCD_write_scm_XY(1,0,scm);
		_delay_ms(100);
}
...
void I2C_LCD_write_scm_XY(uint8_t row, uint8_t col, char *scm)
{
	I2C_LCD_goto_XY(row, col);
	I2C_LCD_write_string(scm);
}